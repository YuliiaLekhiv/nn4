...
# Initialize CNN-LSTM-DNN model
model = Sequential([
    Conv1D(filters=64, kernel_size=2, activation='relu', input_shape=(n_steps, 4)),
...
for i in range(len(X_test)):
    ...
    # Make prediction
    yhat = model.predict(X_test[i].reshape(1, n_steps, 4), verbose=0)
...
plt.title('1D-CNN-LSTM-DNN: Predicted vs Real')
plt.legend()
plt.show()
