...
# Reshape y for inverse transform later
...
for i in range(len(X_test)):
    ...
    if i + n_steps < len(test):
        ...
        X_train = np.append(X_train, X_test[i].reshape(1, -1).reshape(1, n_steps, 4), axis=0)
...
# Calculate metrics
rmse = np.sqrt(mean_squared_error(y_test[:, 3], predictions[:, 3]))
mse = mean_squared_error(y_test[:, 3], predictions[:, 3])
mae = mean_absolute_error(y_test[:, 3], predictions[:, 3])
...
plt.title('Random Forest Regression: Predicted vs Real')
plt.legend()
plt.show()
